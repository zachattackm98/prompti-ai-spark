      ...
      <Card className="bg-gradient-to-br from-slate-800/50 to-purple-900/30 border-purple-500/30 p-4 sm:p-6">
                ...
                >
                  ...
                  <SelectContent className="bg-slate-800 border-purple-500/30 z-50">
                    {musicGenres.map((genre) => (
                      <SelectItem key={genre.value} value={genre.value} className="text-white hover:bg-slate-700 focus:bg-slate-700">
                        {genre.label}
                      </SelectItem>
                    ))}
                  </SelectContent>
                ...
                >
                  ...
                  <SelectContent className="bg-slate-800 border-purple-500/30 z-50">
                    {soundEffect.map((effect) => (
                      <SelectItem key={effect.value} value={effect.value} className="text-white hover:bg-slate-700 focus:bg-slate-700">
                        {effect.label}
                      </SelectItem>
                    ))}
                  </SelectContent>
                ...
                >
                  ...
                  <SelectContent className="bg-slate-800 border-purple-500/30 z-50">
                    {ambienceTypes.map((amb) => (
                      <SelectItem key={amb.value} value={amb.value} className="text-white hover:bg-slate-700 focus:bg-slate-700">
                        {amb.label}
                      </SelectItem>
                    ))}
                  </SelectContent>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-3 sm:gap-0 mt-6 sm:mt-8">
          <Button
            variant="outline"
            onClick={onPrevious}
            className="border-purple-500/30 text-purple-300 hover:bg-purple-900/20 w-full sm:w-auto h-11 sm:h-10 text-sm sm:text-sm order-2 sm:order-1"
          >
          ...
          <Button
            onClick={onNext}
            className="bg-purple-600 hover:bg-purple-700 text-white w-full sm:w-auto h-11 sm:h-10 text-sm sm:text-sm order-1 sm:order-2"
          >
            Continue
