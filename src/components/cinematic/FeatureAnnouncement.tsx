...
const FeatureAnnouncement = ({ userTier, className = '' }: FeatureAnnouncementProps) => {
  ...
  return (
    ...
    >
      <div className={`flex items-center ${isMobile ? 'flex-col space-y-3' : 'gap-3'}`}>
        <div className="flex items-center gap-2 text-green-300">
          <CheckCircle className="w-4 h-4" />
          <span className="text-sm font-medium">All Features Unlocked</span>
        </div>
        <div className={`flex items-center text-sm text-gray-300 ${isMobile ? 'flex-col space-y-2' : 'gap-4'}`}>
