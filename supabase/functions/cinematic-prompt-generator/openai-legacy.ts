...
import { buildSystemPrompt, parsePromptResponse } from './prompt-builder-legacy.ts';

const openAIApiKey = Deno.env.get('OPENAI_API_KEY');

export async function generatePromptWithOpenAILegacy(request: PromptRequest): Promise<GeneratedPrompt> {
  if (!openAIApiKey) {
